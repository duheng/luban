...
class VersionCompilerPlugin {
    ...
    async writeFilemappingsion(fileList) {
        ...
        const staticPath =  this.config.static[process.env.NODE_ENV]
        ...
        fileList.forEach((fileName) => {
            if (/\.(js|css)/.test(fileName)) {
                ...
                if (matchInfo) {
                     ...
                     const __verPath = path.resolve(`${this.config.base}/ver`, name + '.ver')
        ...
        try {
          ...
          const repeatVersion = fileName => {
            ...
            chunks.push(`${staticPath}${projectName}/${build}/${baseName}@${version}${ext}`)
          }
          
          const verDirs = path.resolve(`${base}/ver`)
        ...
        fs.writeFileSync(path.resolve(`${base}/ver`, 'chunk.json.ver'), chunkMd5, 'utf-8')
        mappings.push(`chunk.json#${chunkMd5}`)
        fs.writeFileSync(path.resolve(`${base}/ver`, 'versions.mapping'), mappings.join('\n'), 'utf-8')
        fs.writeFileSync(path.resolve(`${build}/chunk@${chunkMd5}.json`), JSON.stringify(chunks), 'utf-8')
    }
}
